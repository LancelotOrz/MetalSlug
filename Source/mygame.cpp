...
{
	animation.AddBitmap(IDB_c1_left1,RGB(255,255,255));
	/*
...
void CGameStateRun::OnMove()							// 移動遊戲元素
{
	...
	if (background.Top() > SIZE_Y)
		background.SetTopLeft(0 ,-background.Height());
	background.SetTopLeft(background.Left(),background.Top()+1);
	*/
	// 移動背景圖的座標 X軸
	if(right && eraser.GetX1()!=600)//往右
	{
		if (background.Left()+3700 < SIZE_X || eraser.GetX1()!=center)
			eraser.OnMove();
		else
		background.SetTopLeft(background.Left()-speed,background.Top());// 數值"5"將改為角色移動速度
	}
	if(left && eraser.GetX1()!=0)//往左
	{
		if (background.Left() > -10 || eraser.GetX1()!=center)
		eraser.OnMove();
		else
		background.SetTopLeft(background.Left()+speed,background.Top());// 數值"5"將改為角色移動速度
	}
...
void CGameStateRun::OnInit()  								// 遊戲的初值及圖形設定
{
	...
	eraser.LoadBitmap();
	background.LoadBitmap(IDB_map001);					// 載入背景的圖形
	//
	// 完成部分Loading動作，提高進度
	//
	ShowInitProgress(50);
